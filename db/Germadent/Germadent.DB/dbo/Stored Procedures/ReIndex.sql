CREATE PROCEDURE [dbo].[ReIndex] (@DbId SMALLINT = NULL)
        AS
        BEGIN
        /*
        ...
        */
        ...
        WHILE @RowNumber <= @CntRows
                BEGIN
                  ...
                  --Если степень фрагментации менее 25%, выполняем реорганизацию индекса
                  IF @AvgFrag < 25
                     BEGIN
                  ...
                  --Если степень фрагментации более 25%, выполняем перестроение индекса
                  IF @AvgFrag >= 25
                     BEGIN
                        --Формируем строку инструкции и выполняем ее
                        SELECT @Command = 'ALTER INDEX [' + @IndexName + '] ON ' + '[' + @SchemaName + ']' 
