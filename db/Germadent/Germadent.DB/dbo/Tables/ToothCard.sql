CREATE TABLE [dbo].[ToothCard] (
    ...
    CONSTRAINT [FK_ToothCard_Product] FOREIGN KEY ([ProductID]) REFERENCES [dbo].[Products] ([ProductID]),
    CONSTRAINT [FK_ToothCard_WorkOrder] FOREIGN KEY ([WorkOrderID]) REFERENCES [dbo].[WorkOrder] ([WorkOrderID]) ON DELETE CASCADE,
    CONSTRAINT [IX_ToothCard] UNIQUE CLUSTERED ([WorkOrderID] ASC, [ToothNumber] ASC, [PricePositionID] ASC, [ProductID] ASC)
);
...
CREATE NONCLUSTERED INDEX [IX_ToothCard_ProdictID]
    ON [dbo].[ToothCard]([ProductID] ASC);
